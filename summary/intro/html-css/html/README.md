---
description: >-
  Style guide — инструмент, который будет помогать поддерживать единый стиль
  написания кода. Здесь будет собран набор правил того, как пишут код в нашей компании и каких стандартов нужно придерживаться
---

# Style guide: html/css

### HTML

* [Создание html-страницы](sozdanie-html-stranicy.md)
* [Синтаксис](sintaksis.md)

### CSS

* [Синтаксис](sintaksis-css.md)
* [Порядок свойств](poryadok-svoistv.md)
* [Имена классов](imena-klassov.md)
* [Варианты шрифта](varianty-shrifta.md)

